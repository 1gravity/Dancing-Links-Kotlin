 ...
 */
class SudokuMatrix(private val puzzle: Puzzle) {
    ...
    companion object {
         ...
         *     .solve { }
         */
        fun Puzzle.toSudokuMatrix() = SudokuMatrix(this).sudokuMatrix
    ...
    private fun emptyMatrix() = Array(9  * 9 * 9) {
        // 81 cell, 81 row, 81 column, 81 block constraints + 9 constraints per extra region
        // (a Hyper Sudoku e.g. has 4 extra regions, an X Sudoku has 2)
        val extraRegions = puzzle.getRegions(puzzle.extraRegionType).size
    ...
    fun baseMatrix() = emptyMatrix().apply {
        val regions = puzzle.getRegions(puzzle.extraRegionType)
        for (index in 0 until 81) {
            val cell = puzzle.getCell(index)
            for (nr in 1..9) {
                ...
                // block constraint
                val block = puzzle.getRegionAtOrThrow(cell.col, cell.row, RegionType.BLOCK) as Block
                this[row][3 * 81 + block.regionCode * 9 + nr - 1] = true
                // extra region constraint
                regions.forEachIndexed { regionIndex, region ->
                    if (region.contains(puzzle.getCell(index))) {
